...
class PostReplyView(View):
    """View for posting replies to threads."""
    ...
    def post(self, request):
        ...
        slack_service = SlackService(token.token)
        result = slack_service.post_reply(channel_id, thread_ts, reply_text)
        ...
        if result['success']:
            ...
            messages.error(request, f'Failed to post reply: {result["error"]}')

        return redirect('home')


class MessagesView(View):
    """View for getting channel messages via AJAX."""

    def get(self, request):
        token_id = request.GET.get('token_id')
        channel_id = request.GET.get('channel_id')

        if not all([token_id, channel_id]):
        ...
        slack_service = SlackService(token.token)
        messages_result = slack_service.get_channel_history(channel_id, limit=50)

        return JsonResponse(messages_result)
